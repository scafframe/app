...
const SCAFF_AUTH_CONFIG_ = {
  SESSION_KEY_PREFIX: 'SCAFF_SESSION_',
      ...
      try {
        try {
          ...
          const session = {
            ...
            email: user.email,
            issuedAt: Date.now(),
            expiresAt: Date.now() + (SCAFF_AUTH_CONFIG_.SESSION_TTL_SECONDS * 1000)
          };
    
          PropertiesService.getScriptProperties().setProperty(
            SCAFF_AUTH_CONFIG_.SESSION_KEY_PREFIX + token,
...
function isAuthenticatedByToken(tok) {
  try {
    try {
      const normalizedToken = normalizeAuthToken_(tok);
      if (!normalizedToken) return false;
  
      const key = SCAFF_AUTH_CONFIG_.SESSION_KEY_PREFIX + normalizedToken;
      ...
      try {
        ...
        const expiresAt = session && Number(session.expiresAt);
        if (!session || typeof session !== 'object' || !String(session.userId || '').trim() ||
...
function resolveAuthTokenFromPayload_(payloadOrToken) {
  if (typeof payloadOrToken === 'string') {
    return normalizeAuthToken_(payloadOrToken);
  }
  
  if (typeof payloadOrToken === 'object' && payloadOrToken !== null) {
    return normalizeAuthToken_(payloadOrToken._authToken || payloadOrToken.tok);
  }
  
  return null;
}

/** Normaliza tokens recebidos por URL, payload ou chamadas internas. */
function normalizeAuthToken_(token) {
